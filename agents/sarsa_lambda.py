...
class SARSALambdaAgent:
    def __init__(
        ...
        self.E = np.zeros((self.n_states, self.n_actions), dtype=np.float64)
    ...
    def _update_trace(self, s_idx, action):
        ...
        for ep in range(self.episodes):
            ...
            for step in range(self.max_steps):
                ...
                if done:
                    ...
                    td_target = reward + self.gamma * self.Q[ns_idx, next_action]

                td_error = td_target - self.Q[s_idx, action]

                self._update_trace(s_idx, action)
